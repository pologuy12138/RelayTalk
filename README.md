...
environment:
  ...
  VIDEO_HEIGHT: "1080"
  VIDEO_FPS: "24"
  VIDEO_MAX_BITRATE: "8000000"
  VIDEO_START_BITRATE: "5000000"
  VIDEO_MIN_BITRATE: "3000000"
...
| `VIDEO_WIDTH` | `1920` | 摄像头采集目标宽度 |
| `VIDEO_HEIGHT` | `1080` | 摄像头采集目标高度 |
| `VIDEO_FPS` | `24` | 摄像头采集和发送目标帧率 |
| `VIDEO_MAX_BITRATE` | `8000000` | 视频最大发送码率，单位 bps |
| `VIDEO_START_BITRATE` | `5000000` | Chrome 等浏览器可参考的视频起步码率，单位 bps |
| `VIDEO_MIN_BITRATE` | `3000000` | Chrome 等浏览器可参考的视频最低码率，单位 bps |
| `VIDEO_DEGRADATION` | `maintain-resolution` | 拥塞时优先保持分辨率，还是优先保持帧率 |
| `VIDEO_PREFER_CODEC` | `H264` | 优先使用的视频编码，手机硬编通常对 H264 更友好 |
| `VIDEO_STRICT` | `true` | 严格要求采集目标分辨率和帧率；设为 `false` 时允许浏览器选择接近值 |

视频质量参数在 `public/index.html` 里：

```javascript
video:mediaVideoConstraints(vc)
params.encodings[0].maxBitrate=vc.maxBitrate;
```

默认配置是严格 `1080p / 24fps`。如果某台设备不支持这个采集规格，页面会直接报错；可以把 `VIDEO_STRICT` 改成 `false`，允许浏览器选择接近的分辨率和帧率。

## 画质与卡顿说明

RelayTalk 默认强制走 TURN TCP。这个方案的优点是连通性通常更稳，缺点是视频链路会更容易触发 WebRTC 拥塞控制。

WebRTC 会根据网络状态自动调节码率。严格 1080p24 模式会尽量避免分辨率被悄悄降级，但浏览器仍可能在以下情况出现卡顿、丢帧或码率不足：
...
```text
CAM:1920x1080@24 | TX:1920x1080@24 | RX:1080x1920@24 | Q:bandwidth
```
...
1080p24 对 TURN TCP 的带宽和稳定性要求更高。若页面底部出现 `Q:bandwidth`，继续提高码率通常只会更卡；此时应降低帧率或分辨率，或者改用可用的 TURN UDP / 直连路径。
